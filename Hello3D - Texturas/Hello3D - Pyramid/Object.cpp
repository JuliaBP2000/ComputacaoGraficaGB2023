...
void Object::initialize(string filePath,string textureFile, Shader* shader, glm::vec3 position, glm::vec3 scale, float angle, glm::vec3 axis)
{
...
vector<string> Object::loadMtl(string texturePath) {
		...
		if (mtlStream.is_open()) {
			...
			while (getline(mtlStream, line)) {
				...
				if (prefix == "Ka") {
					float ka;
					ss >> ka; 
					this->shader->setFloat("ka", ka);
				}
				if (prefix == "Kd") {
					float kd;
					ss >> kd;
					this->shader->setFloat("kd", kd);
				}
				if (prefix == "Ks") {
					float ks;
					ss >> ks;
					this->shader->setFloat("ks", ks);
				}
				if (prefix == "Ns") {
					float q;
					ss >> q;
					this->shader->setFloat("q", q);
				}
...
vector<string> loadImages(string imagesPath){
	vector<string> imageFiles;

	ifstream mtlStream(imagesPath);
	if (mtlStream.is_open()) {
		string line;
		while (getline(mtlStream, line)) {
			stringstream ss(line);
			...
			if (prefix == "image") {
				string textureFile;
				ss >> textureFile;
				imageFiles.push_back(textureFile);
			}
		}
		mtlStream.close();
	}
...
{
	//vector<string> texNames = loadImages("scene.txt");
	vector<string> texNames = loadMtl(texturePath);
	int iTexture = 0;
