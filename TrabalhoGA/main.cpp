	...
	while (!glfwWindowShouldClose(window))
	{
		...
		if (objects.size() > 0) {
			if (displayAllObjects) {
				...
				if (rotateX)
				{
					objects[currentObjectIdx].rotate(angle, glm::vec3(1.0f,0.0f,0.0f));
				}
				else if (rotateY)
				{
					objects[currentObjectIdx].rotate(angle, glm::vec3(.0f, 1.0f, 0.0f));;
				}
				else if (rotateZ)
				{
					objects[currentObjectIdx].rotate(angle, glm::vec3(.0f, 0.0f, 1.0f));;
				}

				if (moveUp) {
					objects[currentObjectIdx].scaleUp();
					moveUp = false;
				}
				else if (moveDown) {
					objects[currentObjectIdx].scaleDown();
					moveDown = false;
				}
				else if (moveRight) {
					objects[currentObjectIdx].move(glm::vec3(objects[currentObjectIdx].position.x + 0.05f, objects[currentObjectIdx].position.y, objects[currentObjectIdx].position.z));
					moveRight = false;
				}
				else if (moveLeft) {
					objects[currentObjectIdx].move(glm::vec3(objects[currentObjectIdx].position.x - 0.05f, objects[currentObjectIdx].position.y, objects[currentObjectIdx].position.z));
					...
					objects[currentObjectIdx].move(glm::vec3(objects[currentObjectIdx].position.x , objects[currentObjectIdx].position.y, objects[currentObjectIdx].position.z - 0.05f));
					moveFront = false;
				}
				else if (moveBack) {
					objects[currentObjectIdx].move(glm::vec3(objects[currentObjectIdx].position.x, objects[currentObjectIdx].position.y, objects[currentObjectIdx].position.z + 0.05f));
		...
		//{
			...
			VAO = loadSimpleObj(modelsPath[2], nVertices);
			object.initialize(VAO, nVertices, &shader);
			objects.push_back(object);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos)
{
	...
	cameraPos += cameraFront * static_cast<float>(offsety) * cameraSpeed;
}
